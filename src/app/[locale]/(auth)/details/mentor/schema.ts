...
const mentorSchema = z.object({
  ...
  method_of_communication: z
    .array(z.string())
    .min(1, { message: 'At least one personal interest is required' }),
  availability: z.string().min(1, 'Availability is required'),
  mentorship_goals: z.string().min(1, 'Mentorship Goals are required'),
  maximum_number_of_mentees: z
    .string()
    .min(1, 'Maximum Number of Mentees must be at least 1'),

  previous_mentoring_experience: z
    .string()
    .min(1, 'Previous Mentoring Experience is required'),
